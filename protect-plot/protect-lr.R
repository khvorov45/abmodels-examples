...
#' Generates the plot
#'
#' @param lr_fit Predictions from the logistic model
...
# Fitted values. Run `fit-lr.R` in the `model-fit` folder to
# generate `predict-lr.csv`
lr_fit <- read_csv(file.path(fit_dir, "predict-lr.csv"), col_types = cols())

# Summary of fitted values via bootstrap. Generated by `boot-summ-lr.R`
lr_fit_boot <- read_csv(
  file.path(fit_dir, "predict-boot-lr.csv"),
  col_types = cols()
)
...
iwalk(
  list(
    "protect-lr" = lr_plot,
    "protect-boot-lr" = lr_plot_boot_og,
    "protect-boot-lr-rel" = lr_plot_boot_rel
  ),
  save_plot
)
